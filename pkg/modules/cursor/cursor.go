...
// - Cursor extensions installed for each user on disk.
// Relevant fields:
...
func (m *CursorModule) GetName() string {
	return m.Name
}

func (m *CursorModule) GetDescription() string {
	return m.Description
}

func (m *CursorModule) Run(params mod.ModuleParams) error {
	// Check common locations for Cursor extensions
	possibleLocations := []string{
		"/Users/*/Library/Application Support/Cursor/User/extensions",
		"/Users/*/.cursor/extensions",
	}

	for _, locationPattern := range possibleLocations {
		locations, err := filepath.Glob(locationPattern)
		if err != nil {
			params.Logger.Error("Error listing Cursor extension locations: %v", err)
			continue
		}

		for _, location := range locations {
			err = collectCursorExtensions(location, m.GetName(), params)
			if err != nil {
				params.Logger.Error("Error when collecting Cursor extensions: %v", err)
			}
		}
	}
	return nil
...
func collectCursorExtensions(location string, moduleName string, params mod.ModuleParams) error {
	...
	w, err := utils.NewDataWriter(params.LogsDir, outputFileName, params.ExportFormat)
	if err != nil {
